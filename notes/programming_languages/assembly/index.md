- [gdb tricks](programming_languages/assembly/gdb_tricks.md)
- [intel x64 registers](programming_languages/assembly/intel_x64_registers.md)
- [sections](programming_languages/assembly/sections.md)
- [xchg rax,rax](programming_languages/assembly/xchg_rax_rax.md)

## Compilers

- [avr gcc](programming_languages/assembly/avr_gcc.md)

## Instruction Sets

- [intel x64 instruction set](programming_languages/assembly/intel_x64_instruction_set.md)
- [avr instruction set](programming_languages/assembly/avr_instruction_set.md)

## Sandboxes

- [x64 sandbox](programming_languages/assembly/x64_sandbox.md)
- [avr sandbox](programming_languages/assembly/avr_sandbox.md)
