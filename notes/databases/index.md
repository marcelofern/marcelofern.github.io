# Postgres

## Constraints

- [SET CONSTRAINTS](databases/postgres/set_constraints.md)
- [SET NOT NULL](databases/postgres/set_not_null.md)
- [EXCLUDE USING](databases/postgres/exclude_using.md)

## Outages

- [FKs without downtime](databases/postgres/fks_without_downtime.md)
- [Long Transactions](databases/postgres/long_transactions.md)

## Indexing

- [Indexing for LIKE](databases/postgres/indexing_for_like.md)
- [Concurrent index during vacuum](databases/postgres/concurrent_index_during_vacuum.md)
- [Postgres BRIN index](databases/postgres/postgres_brin_index.md)
- [No Automatic Index on FKs](databases/postgres/no_automatic_index_on_fks.md)
- [DROP INDEX CONCURRENTLY](databases/postgres/drop_index_concurrently.md)

## Planning

- [A PSQL Planner Primer](databases/postgres/a-psql-planner-primmer.md)

## Locks

- [`lock_timeout`](databases/postgres/lock_timeout.md)
- [Locks cheatsheet](databases/postgres/locks_cheatsheet.md)
- [Print lock for query](databases/postgres/print_lock_for_query.md)

## Testing

- [Postgres Docker Container](databases/postgres/postgres_docker_container.md)

## Performance

- [OR queries](databases/postgres/or_queries.md)

## Internals

- [Profiling Postgres](databases/postgres/profiling_postgres.md)
- [Debugging Postgres](databases/postgres/debugging_postgres.md)
