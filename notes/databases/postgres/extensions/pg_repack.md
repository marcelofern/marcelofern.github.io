...
LOG:    (param:0) = 2093608
LOG:    (param:1) = (null)
DEBUG: index[0].target_oid      : 2093614
DEBUG: index[0].create_index    : CREATE UNIQUE INDEX index_2093614 ON repack.table_2093608 USING btree (id)
DEBUG: index[1].target_oid      : 2093616
DEBUG: index[1].create_index    : CREATE UNIQUE INDEX index_2093616 ON repack.table_2093608 USING btree (name)
DEBUG: index[2].target_oid      : 2093618
DEBUG: index[2].create_index    : CREATE INDEX index_2093618 ON repack.table_2093608 USING btree (value)
DEBUG: index[3].target_oid      : 2093619
DEBUG: index[3].create_index    : CREATE INDEX index_2093619 ON repack.table_2093608 USING btree (created_at)
LOG: (query) SELECT repack.conflicted_triggers($1)
LOG:    (param:0) = 2093608
LOG: (query) SELECT repack.create_index_type(2093614,2093608)
LOG: (query) SELECT repack.create_log_table(2093608)
...
LOG: (query) SELECT repack.disable_autovacuum('repack.log_2093608')
LOG: (query) BEGIN ISOLATION LEVEL READ COMMITTED
LOG: (query) SELECT pg_backend_pid()
DEBUG: LOCK TABLE public.table_for_repack IN ACCESS SHARE MODE
LOG: (query) SELECT pid FROM pg_locks WHERE locktype = 'relation' AND granted = false AND relation = 2093608 AND mode = 'AccessExclusiveLock' AND pid <> pg_backend_pid()
...
LOG: (query) DELETE FROM repack.log_2093608
...
LOG: (query) INSERT INTO repack.table_2093608 SELECT id,name,value,created_at FROM ONLY public.table_for_repack
LOG: (query) SELECT repack.disable_autovacuum('repack.table_2093608')
LOG: (query) COMMIT
LOG: (query) SELECT 'repack.table_2093608'::regclass::oid
DEBUG: ---- create indexes ----
DEBUG: Have 4 indexes and num_workers=0
DEBUG: set up index_jobs [0]
DEBUG: target_oid   : 2093614
DEBUG: create_index : CREATE UNIQUE INDEX index_2093614 ON repack.table_2093608 USING btree (id)
LOG: (query) CREATE UNIQUE INDEX index_2093614 ON repack.table_2093608 USING btree (id)
DEBUG: set up index_jobs [1]
DEBUG: target_oid   : 2093616
DEBUG: create_index : CREATE UNIQUE INDEX index_2093616 ON repack.table_2093608 USING btree (name)
LOG: (query) CREATE UNIQUE INDEX index_2093616 ON repack.table_2093608 USING btree (name)
DEBUG: set up index_jobs [2]
DEBUG: target_oid   : 2093618
DEBUG: create_index : CREATE INDEX index_2093618 ON repack.table_2093608 USING btree (value)
LOG: (query) CREATE INDEX index_2093618 ON repack.table_2093608 USING btree (value)
DEBUG: set up index_jobs [3]
DEBUG: target_oid   : 2093619
DEBUG: create_index : CREATE INDEX index_2093619 ON repack.table_2093608 USING btree (created_at)
LOG: (query) CREATE INDEX index_2093619 ON repack.table_2093608 USING btree (created_at)
...
LOG: (query) SAVEPOINT repack_sp1
LOG: (query) SET LOCAL lock_timeout = 100
LOG: (query) LOCK TABLE repack.table_2093608 IN ACCESS EXCLUSIVE MODE
LOG: (query) RESET lock_timeout
LOG: (query) SELECT repack.repack_apply($1, $2, $3, $4, $5, $6)
LOG:    (param:0) = SELECT * FROM repack.log_2093608 ORDER BY id LIMIT $1
LOG:    (param:1) = INSERT INTO repack.table_2093608 VALUES ($1.*)
...
LOG:    (param:1) = 2093608
DEBUG: No workers to disconnect.
Process 74897 exited with status = 0 (0x00000000)
```
...
4. Delete any existing entries in the log table since CREATE TABLE ... AS
   SELECT hasn't run yet. `DELETE FROM repack.log_2093608`
