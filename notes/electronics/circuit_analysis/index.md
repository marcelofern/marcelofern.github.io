# Circuit Analysis

## Basic

- [Basic DC circuits (node/supernode and mesh/supermesh)](electronics/circuit_analysis/basic/dc_circuits.md)
- [Linearity and Superposition](electronics/circuit_analysis/basic/linearity_and_superposition.md)
- [Source transformations](electronics/circuit_analysis/basic/source_transformations.md)
- [Thevenin and norton equivalent circuits](electronics/circuit_analysis/basic/thevenin_and_norton_equivalent_circuits.md)
- [Maximum power transfer](electronics/circuit_analysis/basic/maximum_power_transfer.md)
- [Δ-Y Conversion](electronics/circuit_analysis/basic/delta_wye_conversion.md)

## Operational Amplifiers (Op Amp)

- [The ideal operational amplifier](electronics/circuit_analysis/operational_amplifiers/the_ideal_op_amp.md)
- [Negative Feedback vs Positive Feedback](electronics/circuit_analysis/operational_amplifiers/negative_feedback_vs_positive_feedback.md)
- [The practical operational amplifier](electronics/circuit_analysis/operational_amplifiers/practical_op_amp.md)
- [Powering Op Amps](electronics/circuit_analysis/operational_amplifiers/powering_op_amps.md)
- [Common-mode Rejection](electronics/circuit_analysis/operational_amplifiers/common_mode_rejection.md)
- [Comparators](electronics/circuit_analysis/operational_amplifiers/comparators.md)

## Cheatsheet

- Is the circuit very basic? Use node/mesh analysis
- Do you need to find the value for a source for your circuit? Use superposition.

## TODO

- Write a page for an noninverting single supply op amp for AC signals
  (practical electronics for inventors page 650)
